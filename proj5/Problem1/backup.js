...
var S;
var Ka = vec3(0.9, 0.9, 0.9);
var Kd = vec3(0.7, 0.1, 0.6);
var Ks = vec3(0.9, 0.9, 0.9);
var La = vec3(0,2,2);
var shininess = 500;
...
window.onload = function init() {
			...
			//ar tempId = dot(La,n)*Kd*La
			var dotp = dot(La,n);
			
			var r = vec3((n[0] * dotp * 2) - La[0], (n[1] * dotp * 2) - La[1], (n[2] * dotp * 2) - La[2]);

			var rv = dot(r,eye)^shininess;
...
function mousemove(event){ 
	if(tracking && event.buttons===1){
		var p1 = lastVector;
  		...
  		n = cross(p1,p2);
		if(length(n)!=0){
			theta = 5*Math.asin(length(n))*180/Math.PI;
